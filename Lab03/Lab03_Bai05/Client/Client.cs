...
namespace Bai05
{
    public partial class ClientForm : Form
    {
        ...
        public ClientForm()
        {
            ...
            DatabaseHelper.InitializeDatabase(_dbPath); // tạo database
        }
        ...
        {
            DisconnectFromServer(); // ngắt kết nối
            _cts?.Dispose();
        }

        // -- KẾT NỐI VÀ NGẮT SERVER --
        private void ConnectToServer()
        {
            try
            {
                ...
                _ = Task.Run(() => ReceiveLoopAsync(_cts.Token)); // nhận dữ liệu

                MessageBox.Show($"Đã kết nối {_serverIP}:{_serverPort}!", "Thành công");
                SendRequestAsync("GetFoods"); // xin danh sách
            }
            catch (Exception ex)
            {
        ...
        // -- THÊM MÓN ĂN --
        private void AddFood(bool shareToServer)
        {
            ...
            try
            {
                // Luôn lưu vào database local
                SaveToLocalDatabase(ten, nguoi, quyen, _selectedImagePath);

                // Nếu chọn chia sẻ thì gửi lên server
                if (shareToServer)
                {
                    SendToServerAsync(ten, nguoi, quyen, _selectedImagePath);
                    MessageBox.Show("Đã thêm món ăn và chia sẻ lên cộng đồng!", "Thành công");
                }
                else
                {
        ...
        private void SaveToLocalDatabase(string tenMon, string hoVaTen, string quyenHan, string imagePath)
        {
            ...
            File.Copy(imagePath, destPath, true); // copy ảnh vào thư mục client
        ...
        private string SaveBase64ImageToLocal(string base64Image, string tenMon)
        {
            try
            {
                ...
                string localPath = Path.Combine(_imageFolder, fileName);

                byte[] imageBytes = Convert.FromBase64String(base64Image);
                // tạo tên ảnh unique + lưu vào folder client
        ...
        private void ShowResult(string ten, string nguoi, string hinhAnhOrBase64)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(hinhAnhOrBase64))
                {
                    if (File.Exists(hinhAnhOrBase64))
                    {
                        using (var fs = new FileStream(hinhAnhOrBase64, FileMode.Open, FileAccess.Read))
                        {
                    ...
                    {
                        pictureBoxResult.Image = Base64ToImage(hinhAnhOrBase64);
                    }
                }
                else
                {
        ...
        {
            AddFood(shareToServer: false);
        }
        private void btnAddCommunity_Click(object sender, EventArgs e)
        {
            if (_tcpClient?.Connected != true)
            {
                MessageBox.Show("Vui lòng kết nối Server trước khi chia sẻ!", "Lỗi");
                return;
            }

            AddFood(shareToServer: true);
        }
        private void btnMyFoods_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MessageBox.Show($"Đã tải {_myFoods.Count} món ăn cá nhân!", "Thành công");
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Lỗi: {ex.Message}", "Lỗi");
        ...
        private void btnRandomCommunity_Click(object sender, EventArgs e)
        {
            ...
            string localImagePath = SaveBase64ImageToLocal(food.HinhAnh, food.TenMonAn);

            ShowResult(food.TenMonAn, food.NguoiDang, localImagePath);
